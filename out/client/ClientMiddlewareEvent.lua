...
--* @internal 
local ClientMiddlewareEvent
do
	ClientMiddlewareEvent = {}
	function ClientMiddlewareEvent:constructor(middlewares)
		if middlewares == nil then
			middlewares = {}
		end
		self.middlewares = middlewares
	end
	function ClientMiddlewareEvent:_use(middleware)
		local _exp = (self.middlewares)
		table.insert(_exp, middleware)
	end
	function ClientMiddlewareEvent:_processMiddleware(callback)
		local _binding = self
...
local default = ClientMiddlewareEvent
return {
	default = default,
}
